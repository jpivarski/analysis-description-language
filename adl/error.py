...
class ADLSourceError(ADLError):
    def __init__(self, message, code, lineno, col_offset, lineno2=None, col_offset2=None):
...
class ADLNodeError(ADLSourceError):
    def __init__(self, message, node=None, wide=False):
        if node is None or node.code is None:
            ...
            ADLSourceError.__init__(self, message, node.code, node.lineno, node.col_offset, node.lineno2, node.col_offset2)
